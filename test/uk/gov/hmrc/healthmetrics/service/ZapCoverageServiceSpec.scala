...
import uk.gov.hmrc.healthmetrics.model.{Environment, ZapCoverageRequest, ZapCoverageResult}
...
class ZapCoverageServiceSpec
  ...
  "ZapCoverageService.calculateZapCoverage" should:
    "evaluate coverage for all public routes" in new Setup:
      when(serviceConfigsConnector.frontendRoutes(eqTo("test-service"), eqTo(Environment.Production))(using any[HeaderCarrier]))
        .thenReturn(Future.successful(Seq(FrontendRoute("/test-service", isRegex = false))))

      when(serviceConfigsConnector.appRoutes(eqTo("test-service"), eqTo("0.1.0"))(using any[HeaderCarrier]))
