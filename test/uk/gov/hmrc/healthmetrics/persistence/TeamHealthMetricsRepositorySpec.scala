...
import uk.gov.hmrc.healthmetrics.model.{HealthMetric, TeamHealthMetricsHistory, TeamName}
...
class TeamHealthMetricsRepositorySpec
  ...
  "getHealthMetrics" should:
    val metrics1 = TeamHealthMetricsHistory(team1, LocalDate.parse("2025-05-09"), Map.empty[HealthMetric, Int])
    val metrics2 = TeamHealthMetricsHistory(team1, LocalDate.parse("2025-06-10"), Map.empty[HealthMetric, Int])
    val metrics3 = TeamHealthMetricsHistory(team1, LocalDate.parse("2025-06-11"), Map.empty[HealthMetric, Int])
    val metrics4 = TeamHealthMetricsHistory(team2, LocalDate.parse("2025-06-09"), Map.empty[HealthMetric, Int])

    "return health metrics for a team between given dates" in:
      repository.collection.insertMany(Seq(metrics1, metrics2, metrics3, metrics4)).toFuture().futureValue

      repository.getHealthMetrics(
        teamName = team1
      , from     = LocalDate.parse("2025-05-01")
      , to       = LocalDate.parse("2025-07-01")
      ).futureValue shouldBe Seq(
        TeamHealthMetricsHistory(team1, LocalDate.parse("2025-05-09"), Map.empty[HealthMetric, Int])
      , TeamHealthMetricsHistory(team1, LocalDate.parse("2025-06-10"), Map.empty[HealthMetric, Int])
      , TeamHealthMetricsHistory(team1, LocalDate.parse("2025-06-11"), Map.empty[HealthMetric, Int])
      )
