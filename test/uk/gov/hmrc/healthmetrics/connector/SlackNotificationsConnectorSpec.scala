...
class SlackNotificationsConnectorSpec
  extends AnyWordSpec
    ...
    "SlackNotificationsConnector" should:
      "use internal auth" in:
        ...
        val configuration =
          Configuration(
            ...
            "microservice.services.slack-notifications.authToken" -> "token"
          )
        ...
        val slackMessage =
          SlackNotificationsConnector.Request(
            channelLookup = SlackNotificationsConnector.ChannelLookup.ByGithubTeam(teamName = TeamName("PlatOps")),
