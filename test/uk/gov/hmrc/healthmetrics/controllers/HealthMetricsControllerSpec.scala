...
import uk.gov.hmrc.healthmetrics.model.{DigitalService, HealthMetric, TeamHealthMetricsHistory, TeamName}
...
class ServiceStatusControllerSpec
  ...
  private val teamMetrics: Map[HealthMetric, Int] =
    ...
    , HealthMetric.OpenPRForReposOwnedByTeam               -> 1
  ...
  private val digitalServiceMetrics: Map[HealthMetric, Int] =
    Map(
      HealthMetric.OpenPRForReposOwnedByDigitalService     -> 1
    , HealthMetric.LeakDetectionSummaries                  -> 1
    , HealthMetric.ProductionBobbyErrors                   -> 1
    , HealthMetric.LatestBobbyErrors                       -> 1
    , HealthMetric.ProductionBobbyWarnings                 -> 1
  ...
  private val latestTeamMetricsJson =
    """{
      "metrics": {
        ...
        "OPEN_PR_FOR_REPOS_OWNED_BY_TEAM": 1,
  ...
  private val latestDigitalServiceHealthMetricsJson =
    """{
      "metrics": {
        ...
        "CONTAINER_KILLS": 1,
        "PRODUCTION_BOBBY_WARNINGS": 1,
        "OPEN_PR_FOR_REPOS_OWNED_BY_DIGITAL_SERVICE": 1,
        "LATEST_BOBBY_WARNINGS": 1,
        "PLATFORM_INITIATIVES": 1,
        "PRODUCTION_BOBBY_ERRORS": 1,
        "SECURITY_ASSESSMENT_ALERTS": 1
  ...
  private val teamHealthMetricsHistoryJson =
    """[
      {
        "teamName": "Team 1",
        "date": "2025-06-12",
  ...
  "HealthMetricsController.teamHealthMetrics" should:
    "return health metrics history for a team" in:

      val team = TeamName("Team 1")
      val from = LocalDate.parse("2025-06-12")
      val to   = LocalDate.parse("2025-06-14")

      when(mockTeamHealthMetricsRepository.getHealthMetrics(TeamName(eqTo("Team 1")), eqTo(from), eqTo(to)))
        .thenReturn(Future.successful(Seq(
          TeamHealthMetricsHistory(team, LocalDate.parse("2025-06-12"), teamMetrics)
        , TeamHealthMetricsHistory(team, LocalDate.parse("2025-06-13"), teamMetrics)
        )))

      val controller = HealthMetricsController(Helpers.stubControllerComponents(), mockHealthMetricsService, mockTeamHealthMetricsRepository)
      val result     = controller.teamHealthMetrics(team, from = from, to = to)(FakeRequest())
      val bodyText   = contentAsJson(result)
      bodyText mustBe Json.parse(teamHealthMetricsHistoryJson)
